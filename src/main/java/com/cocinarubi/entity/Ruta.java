...
public class Ruta {
    ...
    /** Polígono WKB almacenado como GEOMETRY en MySQL. Usar ST_Contains() en queries nativas. */
    @Column(name = "boundary", nullable = false, columnDefinition = "GEOMETRY")
    private Geometry boundary;

    /** 1 = ruta activa y disponible para asignar pedidos. */
    @Column(name = "is_active", nullable = false)
    private boolean isActive;

    /** Costo base de envío para esta zona, en pesos. */
    @Column(name = "tarifa_envio", nullable = false, precision = 6, scale = 2)
    private BigDecimal tarifaEnvio;

    /** Tiempo estimado de entrega en minutos. Puede ser nulo si no está definido. */
