...
public class PedidoConfirmationImp implements ConfirmationStrategy<PedidoRequestDTO> {
    ...
    public void validarPost(PedidoRequestDTO dto) {
        ...
        validarTarifaDomicilioCocina(dto);
    }
    ...
    private void validarTarifaDomicilioCocina(PedidoRequestDTO dto) {
        if (dto.getPedidoCreadoDesde() != PedidoCreadoDesde.COCINA
                || dto.getTipoPedido() != TipoPedido.DOMICILIO) return;
        Ruta ruta = rutaRepository.findById(dto.getPedidoDomicilioCocina().getIdRuta())
                .orElseThrow(() -> new BusinessException(
                        "La ruta " + dto.getPedidoDomicilioCocina().getIdRuta() + " no existe",
                        HttpStatus.BAD_REQUEST, ErrorCode.VALIDACION));
        if (ruta.getTarifaEnvio().compareTo(dto.getPedidoDomicilioCocina().getTarifa()) != 0) {
            throw new BusinessException(
                    "La tarifa enviada no coincide con el de la ruta del cliente, estás seguro de continuar?",
                    HttpStatus.CONFLICT, ErrorCode.CONFIRMATION);
        }
    }
}
